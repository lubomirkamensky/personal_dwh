CREATE OR REPLACE VIEW pdwh_mart_weather.va_all_measures_monthly4year AS
SELECT
  Month_id,
  Actual_Rain,
  ...
  Records_count
FROM
    pdwh_mart_weather.v_all_measures_monthly4year
UNION ALL
SELECT
  adj.Month_id,
  adj.Actual_Rain,
  ...
  adj.Records_count
FROM
    pdwh_mart_weather.t_adj_all_measures_monthly adj
INNER JOIN
    pdwh_mart_weather.t_loaded_periods tm
    ON adj.Month_id >= LEFT(tm.Day_Id,4)*100