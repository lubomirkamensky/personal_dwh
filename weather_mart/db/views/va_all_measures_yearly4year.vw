CREATE OR REPLACE VIEW pdwh_mart_weather.va_all_measures_yearly4year AS
SELECT
  Year_id,
  Actual_Rain,
  ...
  Records_count
FROM
    pdwh_mart_weather.v_all_measures_yearly4year
UNION ALL
SELECT
  adj.Year_id,
  adj.Actual_Rain,
  ...
  adj.Records_count
FROM
    pdwh_mart_weather.t_adj_all_measures_yearly adj
INNER JOIN
    pdwh_mart_weather.t_loaded_periods tm
    ON adj.Year_id >= LEFT(tm.Day_Id,4)
;