CREATE VIEW pdwh_mart_weather.vtu_all_measures_yearly AS
SELECT
  Year_id,
  Actual_Rain,
  ...
  Records_count
FROM
    pdwh_mart_weather.t_all_measures_yearly
UNION ALL
SELECT
  Year_id,
  Actual_Rain,
  ...
  Records_count
FROM
    pdwh_mart_weather.t_adj_all_measures_yearly
UNION ALL
SELECT
  CAST(LEFT(Month_id, 4) AS INT) AS Year_id,
  Actual_Rain,
  Relative_Pressure_MIN,
  Relative_Pressure_MAX,
  Relative_Pressure_AVG,
...
UNION ALL
SELECT
  CAST(LEFT(Day_id, 4) AS INT) AS Year_id,
  SUM(Actual_Rain),
...
UNION ALL
SELECT
  CAST(LEFT(Hour_id, 4) AS INT) AS Year_id,
  SUM(Actual_Rain),
  SUM(Relative_Pressure_MIN),
  SUM(Relative_Pressure_MAX),
  SUM(Relative_Pressure_AVG),
